...
export default function AuthScreen() {
    ...
    async function signUpWithEmail() {
        ...
        if (data.session) {
            ...
            Alert.alert(t('auth.check_inbox'), t('auth.verify_email'));
            setLoading(false);
        }
    }
    ...
    return (
        <SafeAreaView className="flex-1 bg-background px-6 justify-center">
            <View>
                <Text className="text-4xl font-bold text-text mb-2">{t('auth.register_title')}</Text>
                ...
                <View className="space-y-4">
                    ...
                    >
                        {loading ? (
                            ...
                            <Text className="text-black text-center font-bold text-lg">{t('auth.sign_up')}</Text>
                        )}
                    </Pressable>

                    <View className="flex-row justify-center mt-8">
                        <Text className="text-text-secondary">{t('auth.have_account')} </Text>
